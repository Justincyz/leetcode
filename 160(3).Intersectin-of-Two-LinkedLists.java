...
1/15/2020 follow up
现在题目规定O(1) space了，所以只能用第一种办法。
第二次写的时候出了一点小问题，最后循环的时候假设第二次也走到底了，
此时应该是null == null也会退出while loop的，所以不用担心。
(第二次没搞清楚，先用两个循环计数两个链表的长度，然后循环的时候也计算
走过的长度，如果长度长于两个链表的长度说明没有共同的元素)
*/
