...
class Solution {
    public Node flatten(Node head) {
        ...
        while(dummy != null){
            if(dummy.child !=null){
                Node nextLevelTail = findChildLevelTail(dummy.child);
                Node tempNext = dummy.next;
                dummy.next = dummy.child;
                dummy.child.prev = dummy;
                dummy.child = null;
                nextLevelTail.next = tempNext;
                if(tempNext != null)//注意判断下一位是否为空
                    tempNext.prev = nextLevelTail;
    ...
    public Node findChildLevelTail(Node node){
