...
/*
1/15/2020 第三次写这道题，又有一点新的心得。
主要是在helper function中的 for loop. 
我一开始是从0 到 amount/coins[idx], 结果一直TLE, 很费解。后来才发现，
应该反过来从amount/coins[idx] 到0就不会TLE了，因为我们先尽可能多的
去拿数量大的硬币，而不是尽可能少的去拿。
*/
...
class Solution {
    ...
    public void helper(int[] coins, int amount, int num, int idx){
        
        if(amount == 0){
            res = Math.min(res, num);
            return;
        }
        if(amount < 0 || idx <0) return;
        
        for(int i=amount/coins[idx]; i>=0  && num+i <= res;i--){
            helper(coins, amount-i*coins[idx], num+i, idx-1);
        }
