...
series of jumps.  The (1st, 3rd, 5th, ...) jumps in the series are called odd numbered jumps, and the (2nd, 4th, 6th, ...) jumps in the series are called even numbered jumps.
...
*/
/*
思路，这道题考察的重点就是 TreeMap这个数据结构. ceilingEntry(), floorEntry()都是自带的方法去找到最接近当前节点的大值或者小值。两个 Boolean[]代表了Odd jump
和 even jump. 只有odd jump 取higher[j] == true才可以算作valid index.

答案解析给出的详细例子。每一个元素j都有对应的higher[j]和lower[j]，从后往前遍历。
